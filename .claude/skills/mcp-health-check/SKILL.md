...
1. **Delegate to `vaulter-connection-doctor`.** Don't run `system/scripts/check_mcp_health.py` yourself first --
...
3. **If it reports a fix**, tell the user plainly what was actually wrong and what changed --
   ...
   touched `system/mcp_server.py` or anything it imports, remind the user that Claude Desktop needs a
...
- **Not something the user needs to remember to run.** `system/mcp_server.py`'s own MCP `instructions`
