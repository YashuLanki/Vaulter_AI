...
2. **Never touch the real `confidentials/.env` values.** The test clone gets a blank or dummy
...
1. **Fresh-clone setup:** venv creation, `pip install -r requirements.txt` (watch for a
   ...
   `python scripts/setup_wizard.py` end to end. Note every point where the wizard assumes
   something machine-specific.
2. **Path detection:** `config.py` on a simulated other-account layout. The known unclosed risk:
...
3. **First MCP handshake:** from the temp clone, drive a real `python main.py mcp` subprocess
   over stdio (reuse the pattern in `scripts/check_mcp_health.py`, pointed at the clone). A
...
- **Fix directly** (then re-run the failing step to prove it): a real bug in `setup_wizard.py`,
  `config.py` detection, or install documentation — anything in this project's own files.
