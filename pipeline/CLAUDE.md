...
python main.py web-sources               # list configured web scrape sources
...
python-pptx, images via OCR). `proximity_tool.py` builds proximity/places reports —
geodesic distances via geopy plus the Google Places API (`GOOGLE_PLACES_API_KEY`).
`scheduler.py` (APScheduler) automates all of the above —
...
`open_screening_dashboard`, `run_google_places_export`. `MCP_API_KEY` exists in
`config.py` and `main.py` warns if it's unset, but `mcp_server.py` does not actually
enforce it — the server currently runs unauthenticated (acceptable for stdio transport,
must be fixed before ever exposing HTTP/ngrok).
...
`.claude/settings.json` is shared team config: it pre-approves common read-only
commands, requires confirmation before touching `confidentials/`, denies reading
`.env`/`outlook_token.json` values, and runs `.claude/hooks/check_python_syntax.py`
as a PostToolUse hook — every `.py` file Claude edits is immediately syntax-checked
with `py_compile`, and errors are fed back for an instant fix (there is no linter or
test suite in this repo, so this hook is the only automated safety net).
`.claude/settings.local.json` is personal per-machine config and stays gitignored.
