...
REM Finding a "python" on PATH is not the same as having Python. Windows ships

REM a 0-byte App Execution Alias for python.exe that OPENS THE MICROSOFT STORE

REM instead of running anything -- and it sits in a folder that is on PATH by

REM default, so `where python` succeeds on a machine with no Python at all.

REM Setup would then hand the wizard a command that opens a shop. Verified

REM 2026-08-12 that this alias exists here (0 bytes, resolving to a real

REM Python only because one happens to be installed). So each candidate is

REM ASKED TO RUN before it is trusted.
...
REM Second attempt via winget if the direct installer failed. Not redundant:

REM measured 2026-08-12 on a real machine where the downloaded installer was

REM genuine (correct size, valid Python Software Foundation signature) and

REM still failed with code 3 -- Windows could not stage the installer's own

REM files into its package cache. winget then installed the SAME version from

REM the SAME python.org URL successfully. So this rescues a machine whose MSI

REM plumbing rejects the installer when run directly, which is otherwise a

REM dead end for a teammate who cannot install Python by hand.

REM winget ships with Windows 10/11; if it is absent this simply does nothing.
...
if not "%PYEXIT%"=="0" (
    ...
    if not errorlevel 1 (
        ...
        echo That didn't work. Trying a second way, using Windows' own installer...
...
REM Re-check PATH too: winget's install puts python on PATH for NEW windows,

REM but this one already has its environment. The explicit paths above cover

REM that; this catches anything else that appeared.

if not defined PYCMD for %%C in (python py) do call :try_python %%C
...
echo   Both ways of installing it were tried (the installer finished with

echo   code %PYEXIT%) and Python is still not on this computer, so setup

echo   cannot continue. This is a Windows problem rather than anything

echo   wrong with Vaulter AI, and it is usually quicker to install Python
...
REM Accept %1 as the Python command ONLY if it actually runs. Guards against

REM the Microsoft Store alias described at :python_check, and against a broken

REM or half-removed install still sitting on PATH.
