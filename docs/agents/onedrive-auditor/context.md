...
**Why it exists.** `system/pipeline/proximity_tool.py` was found writing `<name>_<timestamp>.xlsx`/`.csv`
...
apart. Fixed to overwrite one file per property instead. The same day, `output/screening/` was
...
**Not every growing file count is the bug.** `system/geo_cache/` is a legitimate cache — more area
...
**Related docs:**
- `CLAUDE.md` — `system/config.py`'s `SHARED_DIR`/`CORPUS_DIR` distinction; `SHARED_DIR` is written to,
