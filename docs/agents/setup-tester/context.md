...
Claude Desktop. Every path in that journey — venv, requirements, setup wizard, `config.py`'s
...
*different* account. `config.py` detects the account root and derives `SHARED_DIR` and
...
(does the wizard create `confidentials/.env`? is a blank one accepted as a working state — yes,
by design?) but never its *contents*. Real values never appear in a test clone, in output, or in
memory entries.

**Related docs:**
- `scripts/setup_wizard.py` — the guided setup this agent exercises.
- `scripts/release.py` / `scripts/apply_update.py` — the update path; the exclusion/preserve
...
- `scripts/check_mcp_health.py` — the handshake pattern to reuse (pointed at the clone) for the
