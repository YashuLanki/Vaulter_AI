...
**What this agent is for.** Checking the MCP *connector itself* -- the `system/mcp_server.py` process
launched by `python system/main.py mcp` -- is it reachable, does it register all its tools, does every
...
is the whole reason this agent's primary tool, `system/scripts/check_mcp_health.py`, insists on spawning
a genuine subprocess rather than importing `system/mcp_server.py` and calling a tool function directly.
...
- `check_system_health`'s own `instructions=` text (in `system/mcp_server.py`'s `create_mcp_server()`)
...
**A real, unexplained wrinkle worth knowing about, not treated as a bug:** every `python system/main.py
...
Grepping `system/main.py`/`system/mcp_server.py` found no `subprocess`/`sys.executable`/`multiprocessing` call
...
**Related docs:**
- `CLAUDE.md` -- `system/mcp_server.py`'s "no background threads" invariant, and the "a crash and a hang
  are indistinguishable from the outside" lesson this agent exists to catch before a user does.
- `system/scripts/check_mcp_health.py` -- the deterministic tool this agent runs first, every time.
