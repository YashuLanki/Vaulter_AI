...
corrupt each other; add a simple update path; and move the Project Master into the shared
folder (it's not private, unlike email) so one export updates everyone, with the local drop as
an override.
...
- `ingestion/watcher.py` — the folder watcher and property/state caches (A2 item).
